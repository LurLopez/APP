...
8. **Asignación de Capital (Bloque 3)**:
   ...
   - **Signos obligatorios**:
    ...
    - **Deuda asumida (no-cash)**: si hubo adquisición material y la deuda del balance AUMENTÓ en parte por deuda preexistente de la empresa adquirida (campo `assumedDebt` > 0), la diferencia se muestra con signo negativo (-) en `Deuda asumida (no-cash)` (>= 50M), corrigiendo la fila `Deuda` para que el cuadre refleje solo la deuda con entrada de caja. La nota debe desglosar el aumento de deuda entre `deuda emitida/amortizada con caja` y `deuda asumida con la compra (no-cash)`, aclarando que esta última no supone entrada de caja y se resta en el cuadre. Si `assumedDebt` es 0 (sin adquisición material o sin deuda asumida detectada), la fila NO se incluye: prohibido inventarla o duplicar con ella la variación de deuda.
    - **Movimientos no monetarios y reclasificaciones**: además de las filas anteriores, se mapean siempre las variaciones que no suponen entrada/salida real de caja y que rompen el cuadre: efectivo restringido/escrow (`Efectivo restringido`), deuda asumida en compras (`Deuda asumida (no-cash)`), efecto divisa sobre la caja y reclasificaciones entre caja e inversiones a corto plazo. Si tras mapearlas el `En total` sigue descuadrado, la verificación indica el importe exacto del desfase y que corresponde a movimientos no monetarios o reclasificaciones a revisar en las notas de flujos y balance; nunca se deja el descuadre sin cifra ni se oculta con una fila genérica sin desglose.
