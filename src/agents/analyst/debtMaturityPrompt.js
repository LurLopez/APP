...
import { maturityWindowBounds } from './debtMaturityFallback.js';
 ...
 */
export function buildDebtMaturityPrompt(fiscalYear, periodEnd) {
  const year = Number(fiscalYear);
  const bounds = Number.isFinite(year) ? maturityWindowBounds(year, periodEnd) : null;
  const windowEnd = bounds?.maxYear ?? (Number.isFinite(year) ? year + 5 : null);
...
{"items":[{"year":2026,"label":"CAD 500M 3.44% senior notes","amount":364.3,"rate":3.44,"type":"Senior Notes"}],"afterYearFive":null}
...
- "afterYearFive": suma del principal que vence ${afterLabel}; null si no consta o no hay vencimientos posteriores. PROHIBIDO copiar el importe del ejemplo (3841.6 u otro): el ejemplo es genérico y no pertenece a este informe.
...
- Ignora las filas sin año de vencimiento (subtotales, "Current maturities of long-term debt", "Total") y las que ya vencieron antes de la fecha de cierre del ejercicio.
