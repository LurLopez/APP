...
- BLOQUE 1 — VENTAS (Cuenta de Resultados):
  ...
  * AJUSTE DE DETERIOROS / IMPAIRMENTS / AMORTIZACIONES (APLICA LA POLÍTICA SECTORIAL INDICADA ARRIBA):
    - El deterioro viene desglosado por naturaleza en la extracción: FONDO DE COMERCIO ("goodwillImpairmentQuarter", "goodwillImpairmentPrevQuarter", "goodwillImpairmentYtd", "goodwillImpairmentPrevYtd") e INTANGIBLES/MARCAS/ACTIVOS ("intangibleImpairmentQuarter", "intangibleImpairmentPrevQuarter", "intangibleImpairmentYtd", "intangibleImpairmentPrevYtd"). Los campos agregados ("impairmentsPrevQuarter", "impairmentsPrevYtd", "impairmentsQuarter", "impairmentsYtd") son el total de ambos y solo se usan si no existe desglose.
    - Solo se suma de vuelta la parte que la POLÍTICA SECTORIAL indique:
      * Deterioro de fondo de comercio: se suma de vuelta si la política lo indica.
      * Deterioro de intangibles/marcas/activos: se suma de vuelta SOLO si la política lo indica; en los sectores que lo mantienen como coste, NO se revierte ni se resta (permanece en la columna Ajustado) y puede mencionarse en la nota explicativa que se mantiene como coste.
    - Si en el ejercicio anterior comparable hubo deterioros de la parte que la política suma de vuelta:
      1. ES OBLIGATORIO SUMAR DE VUELTA dicho deterioro en la columna "Anterior Ajustado" (prevAdjusted) para Beneficio Operativo, EBT y Beneficio Neto.
         * Beneficio Operativo Anterior Ajustado = Beneficio Operativo Anterior Normal + Deterioro Anterior Ajustable (ej. -101M + 1428M = 1327M en KHC 3M).
         * EBT Anterior Ajustado = EBT Anterior Normal + Deterioro Anterior Ajustable (ej. -283M + 1428M = 1145M en KHC 3M).
         * Beneficio Neto Anterior Ajustado = Beneficio Neto Anterior Normal + Deterioro Anterior Ajustable (o neto de impuestos).
      2. NUNCA copies "Anterior Normal" a "Anterior Ajustado" si hubo un deterioro ajustable en el ejercicio anterior. Si el único deterioro del periodo anterior es de la parte NO ajustable (ej. intangibles en tecnología), "Anterior Ajustado" hereda "Anterior Normal" y no se colorea.
      3. Añadir la nota explicativa correspondiente (ej. "*1: El año anterior tuvieron un impairment de 1428M" o, si es solo fondo de comercio, "*1: El año anterior tuvieron un deterioro de fondo de comercio de 1428M").
    * AJUSTE DE ESTE AÑO EN "AJUSTADO":
      - Si este año hubo deterioros de la parte que la política suma de vuelta ("goodwillImpairmentQuarter"/"goodwillImpairmentYtd" o, si procede, "intangibleImpairmentQuarter"/"intangibleImpairmentYtd"), súmalos de vuelta a Beneficio Operativo, EBT y Beneficio Neto en la columna "Ajustado" y pon la nota explicativa correspondiente (ej. "*1: Ha habido un deterioro de fondo de comercio de 6729M").
      - La amortización recurrente de intangibles ("intangiblesAmortizationQuarter" / "intangiblesAmortizationYtd") se suma de vuelta SOLO si la política sectorial lo indica; si la política la mantiene como coste, NO se suma y no genera nota.
      - El deterioro de intangibles/marcas/activos que la política mantenga como coste no se suma, no se resta y no lleva resalte; si es material, se explica en una nota informativa que permanece como coste en la columna Ajustado.
   * IMPUESTOS NORMALIZADOS SOBRE EL EBT AJUSTADO (REGLA DE DESVIACIÓN ±20%):
    ...
    - Ejemplo: EBT reportado 100M y beneficio neto 80M (tipo efectivo 20 %). Si el EBT ajustado es 300M, mantener solo 20M de impuestos es INCORRECTO: impuestos normalizados = 23 % × 300M = 69M → Beneficio Neto Ajustado = 231M.
      ...
      - COHERENCIA OBLIGATORIA TABLA ↔ NOTA FISCAL: el Beneficio Neto Ajustado de la tabla debe ser EXACTAMENTE el resultado que cierra la nota fiscal. Si NO procede normalizar (desviación dentro de ±20 %), Beneficio Neto Ajustado = EBT Ajustado − impuesto reportado, usando la MISMA base del Beneficio Neto Normal (si el Normal es atribuible a la matriz, usa el neto atribuible; nunca mezcles base atribuible con base consolidada) y NUNCA apliques 0,77 en ese caso. Si procede normalizar, Beneficio Neto Ajustado = EBT Ajustado × 0,77.
      - Antes de cerrar el JSON, comprueba que el último término de la igualdad de la nota fiscal coincide dígito a dígito con la celda Ajustado del Beneficio Neto. Una nota que calcula una cifra distinta a la de la tabla invalida el análisis.
      - CUANDO NO SE NORMALIZA Y NO HAY DETERIOROS: si el impuesto reportado está dentro del ±20 % y no hay deterioros ni ajustes que sumar de vuelta en la columna Ajustado, el Beneficio Neto Ajustado debe ser EXACTAMENTE IGUAL al Beneficio Neto Normal (copia el valor, no lo recalcules con el 23 %: eso es un error). Escribe la igualdad usando la misma base de la tabla (si el Normal es atribuible, usa el atribuible). PROHIBIDO que la celda Ajustado equivalga a EBT ajustado × 0,77 si no procede normalizar.
      - UNIDADES: todas las cifras del informe se expresan en MILLONES de USD. Si el estado financiero del filing está en miles, divide las cifras entre 1.000 antes de usarlas. Comprueba el orden de magnitud: un ajuste fiscal no puede superar el propio Cash Flow ni ser desproporcionado frente al EBT; si lo es, has leído la cifra en miles y debes corregirla.
      - BASE DEL BENEFICIO NETO: usa el beneficio neto ATRIBUIBLE a la matriz (net income attributable to…) cuando el filing lo desglose; si solo publica el consolidado, usa ese. La MISMA base debe emplearse en Normal, Ajustado y en las notas fiscales; nunca mezcles consolidado y atribuible (empresas con participaciones no controladoras). SI USAS LA BASE ATRIBUIBLE, la fórmula de la nota es: Beneficio Neto Ajustado = EBT ajustado − impuesto reportado − participación no controladora (NCI) del periodo; escribe el NCI en la igualdad si existe (p. ej. 9319 − 2168 − 125 = 7026) y comprueba la aritmética.
   * Regla de herencia en "Anterior Ajustado" (prevAdjusted): ÚNICAMENTE si en el ejercicio anterior comparable NO hubo ningún ajuste contable documentado ni deterioros ajustables según la política, hereda obligatoriamente el valor de "Anterior Normal" (prevNormal), y calcula SIEMPRE el "% Ajustado" (pctAdjusted). Prohibido poner "—" si prevNormal tiene cifra.
   ...
   * Principio de Resaltado Exclusivo en la Casilla de Origen (Sin Propagación en Cascada): El color y la llamada de nota ("isAdjusted": true, "adjustedNote": "*1") se asignan ÚNICA Y EXCLUSIVAMENTE a la casilla de la métrica donde se origina directamente el ajuste contable:
     - Intangibles / amortización / deterioros (impairments) que la política suma de vuelta: marcar "isAdjusted": true ÚNICAMENTE en "Beneficio Operativo". Aunque EBT y Beneficio Neto varíen matemáticamente en la columna Ajustado por arrastre aritmético, NO llevan resalte ("isAdjusted": false) ni asterisco a menos que contengan un ajuste directo propio. Los deterioros de intangibles/marcas/activos que la política mantiene como coste no se marcan ni generan resalte.
 ...
 - BLOQUE 2 — CASH FLOW:
    ...
    * PERIODO DEL AJUSTE FISCAL: en el horizonte de 3 meses usa SOLO el impuesto pagado DEL TRIMESTRE; si el filing únicamente publica el acumulado, omite el ajuste fiscal en ese horizonte (nunca uses el acumulado como si fuera trimestral). En el acumulado usa el impuesto pagado acumulado.
    * SIGNO DEL AJUSTE FISCAL (convención oficial, no invertir): ajuste = impuestos pagados − impuestos normalizados. Si es negativo (pagó menos), se RESTA; si es positivo (pagó más), se SUMA. No describas el signo al revés.
  ...
  * Métricas obligatorias en orden: Cash Flow, CAPEX, FCF, FCF/Acción, Dividendo, Libre.
    * LA DEDUCCIÓN TRIMESTRAL EN Q2, Q3 Y Q4: NUNCA pongas "—" en los flujos de los últimos 3 meses si dispones de flujos acumulados. Si el informe solo da los flujos acumulados YTD, calcula obligatoriamente la resta:
      Flujo (3M) = Flujo YTD (Qn) - Flujo YTD (Qn-1)
      Usa obligatoriamente las cifras de "deducedQuarterCashFlow" y "previousQuarterCashFlow". Si no dispones de ambas cifras y el filing solo publica el acumulado, deja la columna de 3M como "—" y añade una nota explicando que el informe solo publica el acumulado; queda prohibido copiar el acumulado en la columna trimestral.
     * AMORTIZACIÓN DE INTANGIBLES POR HORIZONTE: si la política sectorial exige sumarla de vuelta, en el horizonte de 3 meses usa la amortización del TRIMESTRE (intangiblesAmortizationQuarter); en el acumulado, la del YTD (intangiblesAmortizationYtd). Nunca uses la acumulada para el trimestre. Lo mismo con el gasto por impuestos: en 3M usa incomeTaxExpenseQuarter (el del trimestre), no el acumulado.
  * Notas de Cash Flow y Resaltado en Cabecera:
    - PROHIBIDO incluir nota al pie de deducción trimestral: La resta entre flujos acumulados es una simple operación matemática ordinaria, no un ajuste de criterio contable. Queda estrictamente prohibido generar notas como "*1: Flujo trimestral deducido del acumulado...".
     - Nota de Cash Flow: La normalización de capital circulante es siempre la Nota 1 ("*1: WC = ...") y la cabecera siempre "Ajustado*1 (WC=valorAjustado)". Si se aplica una normalización fiscal dentro del umbral permitido, su explicación va como NOTA INDEPENDIENTE ("*2: Impuestos: ..."), nunca mezclada dentro de la nota del circulante; la celda de Cash Flow Ajustado se identifica únicamente con el color de la Nota 2, SIN escribir "*2" en el texto de la celda.
      * COHERENCIA NOTA *1 ↔ TABLA: si existe nota *2 (ajuste fiscal), la nota *1 NO puede llamar "Cash Flow Ajustado" al resultado del ajuste de circulante; debe escribir "El Cash Flow tras el ajuste de circulante queda en: ..." (subtotal intermedio) y dejar que la nota *2 cierre la cadena hasta el valor final de la tabla. El único "Cash Flow Ajustado" es el de la tabla.
      * Desglosar la estimación del capital circulante y el ajuste resultante en Cash Flow: el WC teórico se calcula SIEMPRE con el peso agregado histórico del circulante sobre el flujo operativo sin circulante de los últimos 10 ejercicios (WC = peso agregado % × flujo del periodo sin circulante), tanto en consumo defensivo como en tecnología y software, con o sin volumen publicado. La nota debe desglosar el peso agregado, los ratios de cada ejercicio y el ajuste resultante.
      - Resaltado y color coordinado: La cabecera de la columna ajustada debe llevar la llamada a la nota ("Ajustado*1") y el color de la Nota 1 por la intervención de circulante. Las filas de datos permanecen limpias por WC; la celda de Cash Flow Ajustado lleva el color de la Nota 2 (sin llamada de texto en la celda). Cuando existan ajuste fiscal (*2) y deducción de compensación en acciones (*3), ambas partidas modifican la misma celda de Cash Flow Ajustado, por lo que las llamadas *2 y *3 comparten el mismo color de resalte oficial de dicha casilla.
  ...
  * FILAS Y CONVENCIÓN DE SIGNOS:
    ...
    4. "Adquisiciones": Pagos netos por compra de negocios o empresas ("Acquisitions of businesses, net of cash acquired", "Acquisition of business, net of cash acquired", "Payments to acquire businesses", campos "acquisitions..." del JSON, >= 50M). Fila OBLIGATORIA si existe una adquisición material: signo NEGATIVO (-) porque es un uso de capital. Si no hubo adquisiciones o fueron marginales (< 50M), NO incluir esta fila.
     5. "Deuda": Se calcula OBLIGATORIAMENTE comparando la Deuda Total (Deuda a largo plazo + Deuda a corto plazo, excluyendo cuentas a pagar a proveedores que forman parte del Working Capital) directamente en el BALANCE:
        - Deuda Balance = Deuda a largo plazo (Long-Term Debt, incluida la porción corriente) + Deuda a corto plazo (Current debt / Short-Term debt / arrendamientos financieros corrientes) tal como figura en el BALANCE.
        - FUENTE ÚNICA: usa SIEMPRE la deuda del balance. Si el MD&A o el comunicado publican un "Total debt" no-GAAP que no coincide (p. ej. porque excluye arrendamientos financieros o deuda corriente), NO lo uses para la fila, la nota ni el histórico; si lo mencionas como contexto, aclara expresamente qué excluye y por qué difiere.
    ...
    6. "Caja": Se calcula OBLIGATORIAMENTE comparando el Efectivo directamente en el BALANCE:
     ...
     8. "Emisión de preferentes" y "Venta de participaciones": fuentes de capital (signo POSITIVO (+)) por emisión de preferentes o venta de participaciones no controladoras; cada fila solo si el importe es >= 50M.
     9. "Deuda asumida (no-cash)": SOLO si el ejercicio tuvo una adquisición material y la deuda del balance AUMENTÓ en parte por deuda preexistente de la empresa adquirida (campo "capitalAllocationData.assumedDebt", siempre >= 50M). Signo NEGATIVO (-). Si assumedDebt es 0 o no existe, esta fila NO se incluye nunca (prohibido inventarla o duplicar con ella la variación de deuda del balance).
    10. "En total": Última fila obligatoria. Suma algebraica con signo de todas las partidas de la tabla: Libre + Inversiones a corto plazo + Desinversiones + Adquisiciones + Deuda + Caja + Recompras + Emisión de preferentes + Venta de participaciones + Deuda asumida (no-cash).
    * LA TABLA SOLO LLEVA MOVIMIENTOS DE CAJA: el efectivo restringido/escrow y la deuda no monetaria (recompras o amortizaciones anticipadas de deuda con ganancia o pérdida, efecto divisa) NO se pintan como filas. El sistema los explica bajo la tabla, junto al importe del descuadre, cuando el cuadre supera el umbral.
   * FILAS SIN IMPORTE: queda PROHIBIDO incluir filas con valor 0 o "—" en la asignación de capital. Solo "Libre" y "En total" son obligatorias; el resto se incluye únicamente si su importe es material (>= 50M).
   * Verificación de cuadre ("verification"):
     ...
     - Si el descuadre supera ese umbral: "No cuadra: quedan <importe>M sin explicar entre el capital libre y los usos detectados..."; se indica el importe exacto y se explica el descuadre citando los movimientos que no pasan por caja (efectivo restringido, deuda no monetaria, efecto divisa, reclasificaciones caja/inversiones) a revisar en las notas de flujos y balance. Si el filing muestra otras partidas materiales identificables que explican el hueco (emisión de acciones, liquidación de forwards de divisa, aportaciones/retiradas de participaciones, etc.), cítalas con su importe exacto; no uses solo frases genéricas. Nunca se deja el descuadre sin cifra ni sin explicación.
