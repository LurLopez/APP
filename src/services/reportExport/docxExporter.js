...
function docxRichParagraph(text, opts = {}) {
  ...
  return paragraphs.map((paragraph, index) => {
    const pPr = `${opts.pageBreakBefore && index === 0 ? '<w:pageBreakBefore/>' : ''}<w:spacing w:before="${opts.before ?? 0}" w:after="${opts.after ?? 40}"/>`;
    const runs = parseRichSegments(paragraph).map((seg) => docxRun(seg.text, {
      size: opts.size ?? 8.5,
